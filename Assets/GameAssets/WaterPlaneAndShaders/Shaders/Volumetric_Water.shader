Shader "GapperGames/Volumetric_Water"
{
    ...
    Varyings BuildVaryings(Attributes input)
    {
        ...
        VertexDescriptionInputs vdi = BuildVertexDescriptionInputs(input);
        VertexDescription vd = VertexDescriptionFunction(vdi);

        output.positionWS = TransformObjectToWorld(vd.Position);
        output.normalWS = TransformObjectToWorldNormal(vd.Normal);
        output.tangentWS = float4(TransformObjectToWorldDir(vd.Tangent), 1.0);
        output.viewDirectionWS = GetWorldSpaceViewDir(output.positionWS);
        output.positionCS = TransformWorldToHClip(output.positionWS);

    #if defined(LIGHTMAP_ON)
        output.lightmapUV = input.uv1.xy;
    #else
        output.sh = SampleSH(output.normalWS);
    #endif

    output.shadowCoord = TransformWorldToShadowCoord(output.positionWS);
    output.fogFactorAndVertexLight = float4(ComputeFogFactor(output.positionCS.z), 0, 0, 0);
